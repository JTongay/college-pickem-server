...
export class CollegeCrawler {
  ...
  public static Scrape (year: string = '2017', week: string = '1'): void {
    request(`http://www.foxsports.com/college-football/schedule?season=${year}&seasonType=1&week=${week}&group=0`, (err, response, body) => {
      if (err) {
      ...
      const schedule: any[] = _.chunk(teamsFiltered, 3);
      ...
      fs.writeFile(path.join(__dirname, '..', '..', 'src', 'json', `week_${week}_year_${year}_ncaa.json`), JSON.stringify(schedule), 'utf-8', (errorMessage) => {
